...
namespace Gronsfeld
{
    ...
    Gronsfeld::Gronsfeld(const QVector<QString> &abc)
    {
        this->alphabet = abc;
    }

    void Gronsfeld::setKey(const QString &k)
    {
        this->key = k;
    }
    ...
    QString Gronsfeld::encrypt(const QString &message, qsizetype func(qsizetype, qsizetype))
    {
        ...
        {
            qsizetype curr_key = static_cast<qsizetype>(tmp_key.mid(it - message.cbegin() - shift, 1).toInt());
    ...
    QChar Gronsfeld::shift(const QChar c, const qsizetype sh, qsizetype func(qsizetype, qsizetype))
    {
        for (QVector<QString>::iterator it = this->alphabet.begin(); it != this->alphabet.end(); ++it)
        {
            if ((*it).contains(c))
            {
                qsizetype new_index = MathAux::euclidean_remainder<qsizetype>(func((*it).indexOf(c), sh), (*it).length());
    ...
    void Gronsfeld::setKeyMode(const KeyMode k)
    {
        this->key_mode = k;
    }
